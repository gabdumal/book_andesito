...
Dessa vez, vamos adotar a convenção de **colocar** todos os arquivos de código-fonte dentro de uma pasta chamada `src`.
Execute os comandos a seguir para fazê-lo.
...
Aém disso, a [documentação do Clang](https://clang.llvm.org/docs/DiagnosticsReference.html) mostra quais flags estão implementadas no Clang.
...
```cmake title="CMakeLists.txt" frame="code" startLineNumber=28
## Directories
