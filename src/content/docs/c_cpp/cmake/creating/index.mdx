...
Dentro do diretório `oriented_graph`, **crie** os arquivos `main.cpp`, `.clang-tidy` e `.clang-format`.

```zsh title="Fedora, WSL ou Mint" frame="terminal"
touch main.cpp .clang-tidy .clang-format
```

Também crie três pastas: `node`, `edge` e `graph`.

```zsh title="Fedora, WSL ou Mint" frame="terminal"
mkdir -p node edge graph
```

Dentro de cada uma, **crie** conter um arquivo `.cpp` e um arquivo `.hpp` com o mesmo nome da pasta.
