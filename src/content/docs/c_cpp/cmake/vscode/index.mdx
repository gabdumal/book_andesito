...
import vscodeLaunchForOrientedGraph from "/public/code/c_cpp_projects/oriented_graph/.vscode/launch.json?raw";
...
<Figure caption={`Seleção de um kit de configuração no menu do CMake Tools.`}>
  <Image
...
<Figure caption={`Seleção do kit Clang.`}>
  <Image
    src={selectingClang}
...
<Figure caption={`Compilando pela paleta de comandos do Visual Studio Code.`}>
  <Image
...
<Figure caption={`Compilando pelo botão de Build do CMake Tools.`}>
  <Image
...
<Figure caption={`Saída da compilação do projeto no Visual Studio Code.`}>
  <Image
    src={buildOutput}
...
<Figure caption={`Executando o projeto pelo botão de Launch do CMake Tools.`}>
  <Image
...
Caso você tenha seguido as instruções para configurar o perfil `C/C++` do VSCode, como discutido [anteriormente](../../compiler/vscode#configurações), seu ambiente já terá utilidades que facilitam a depuração de um programa.
Entre elas, definimos algumas configurações específicas para o CMake.

Agora, **adicione** um ponto de depuração na linha `12` do arquivo `src/main.cpp`.
Então, clique no botão com o ícone de **besouro** na barra inferior do VSCode.
Ele tem a função de **Debug**.

<Figure caption={`Depurando o projeto pelo botão de Debug do CMake Tools.`}>
  <Image
...
O VSCode abrirá o console de depuração e executará o programa `oriented_graph` em modo de depuração.

<Figure caption={`Tela de depuração do projeto no Visual Studio Code.`}>
  <Image
    src={debugScreen}
    alt={`Tela de depuração do projeto no Visual Studio Code. O programa está parado na linha 12 do arquivo main.cpp. Na parte esquerda da tela, estão as variáveis do programa, e na parte inferior, o console de depuração.`}
  />
</Figure>

### Tarefa personalizada

Se você precisar customizar a tarefa de depuração, como adicionar argumentos ou variáveis de ambiente, você pode fazer isso por meio do arquivo `launch.json`, que fica dentro da pasta `.vscode` do seu projeto.
...
<Code
  ...
  code={vscodeLaunchForOrientedGraph}
  frame="code"
/>
...
A interface de depuração comum do VSCode, que pode ser acessada por meio do atalho de teclado <span style="white-space: nowrap;"><Kbd linux={"Ctrl+Shift+D"} />,</span> passa a funcionar totalmente com o CMake.
