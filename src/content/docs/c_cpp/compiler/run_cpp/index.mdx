...
<Figure caption={`Compilação e execução do programa "Binary Tree".`}>
  <Image
    src={compileRunHello}
...
**Copie** o conteúdo de todos os trechos de código abaixo para os respectivos arquivos criados.
...
<Figure caption={`Compilação e execução do projeto "binary_tree".`}>
  <Image
...
<Figure caption={`Construção do arquivo "tasks.json".`}>
  <Image
...
<Figure caption={`Preenchimento da tarefa de compilação "cpp-build-task".`}>
  <Image
    src={fillBuildTask}
...
```json title=".vscode/tasks.json" frame="code" {5,7,9}
"args": [
  "-fcolor-diagnostics",
  "-fansi-escape-codes",
  "-g",
...
<Figure caption={`Saída da compilação do projeto "binary_tree".`}>
  <Image
    src={localOutput}
