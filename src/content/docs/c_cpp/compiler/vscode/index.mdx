...
Dentro dela, crie um arquivo chamado `tasks.json` e adicione o conteúdo do arquivo [`tasks.json`](https://github.com/gabdumal/book_andesito/tree/main/public/code/c_cpp_projects/linked_nodes/.vscode/tasks.json).

O que ele faz é definir uma tarefa chamada `Clang: build C project (all files in root)` para o Linux e outra para o Windows.
...
Copie o conteúdo do arquivo [`tasks.json`](https://github.com/gabdumal/book_andesito/tree/main/public/settings/vscode/c/c_cpp_profile/tasks.json) e cole no arquivo `tasks.json` aberto.
Lembre-se de salvar.
...
Para facilitar isso, podemos criar um snippet para a tarefa de compilação.
Abra a paleta de comandos com <Keyboard keys={["Ctrl", "Shift", "P"]} /> e pesquise por `Snippets: Configure Snippets`.

Selecione a opção `global.code-snippets`.

Adicione o seguinte código ao final da lista de snippets.
...
"Build tasks": {
    ...
    "body": [
        ...
        "        \"kind\": \"build\"",
        "      },",
        "      \"label\": \"Clang: build C project (all files in root) (Linux)\",",
        "      \"options\": {",
        "        \"cwd\": \"${workspaceFolder}\"",
        "      },",
        "      \"problemMatcher\": [\"$$gcc\"],",
...
Os arquivos modificados do perfil do Visual Studio Code ficarão como no [repositório do livro](https://github.com/gabdumal/book_andesito/tree/main/public/settings/vscode/c_tasks/c_cpp_profile/tasks.json).
