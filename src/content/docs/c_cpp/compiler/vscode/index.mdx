...
import vscodeSnippet from "/public/configuration/vscode/c_cpp/snippets/global.code-snippets?raw";
...
import compileRunLinked from "@assets/content/docs/c_cpp/compiler/vscode/compile_run_linked.png";
...
Digite o nome `global` e pressione <Kbd linux={"Enter"} />.
...
Em seguida, copie o conteúdo a seguir e cole no arquivo `global.code-snippets` que foi aberto.
Lembre-se de **salvar** após a edição.

<Code
  lang="json"
  title="<...>/Code/User/profiles/<ID>/snippets/global.code-snippets"
  code={vscodeSnippet}
  frame="code"
/>
...
Abra o terminal integrado com <Kbd linux={"Ctrl+`"} />, se você tiver configurado esse atalho, ou a partir dos menus `Terminal > New Terminal` e execute o comando abaixo.
...
O que o arquivo faz é definir uma tarefa chamada `Clang: build C project (all files in root)` para o Linux.
...
O executável gerado terá o nome da **pasta** aberta no Visual Studio Code, `linked_nodes`.
...
Seleciona a tarefa `Clang: build C project (all files in root) (Local)`.
...
Então, digite `./linked_nodes` e pressione <Kbd linux={"Enter"} />.
