...
<Figure caption={`Criando um novo perfil chamado "C/C++".`}>
  <Image
    src={createProfile}
...
<Figure caption={`Selecionando o perfil "C/C++".`}>
  <Image
    src={selectProfile}
...
Se você estiver usando o WSL, certifique-se de clicar nos botões **Install in WSL: fedora** para cada extensão da lista de instaladas que oferecer essa opção.
...
<Figure caption={`Abrindo a configuração de snippets do Visual Studio Code.`}>
  <Image
    src={searchSnippets}
...
<Figure caption={`Criando um novo arquivo de snippets.`}>
  <Image
    src={newSnippetsFile}
...
<Figure caption={`Nomeando o arquivo de snippets como "c_cpp".`}>
  <Image
    src={globalSnippets}
...
<Figure caption={`Conteúdo padrão do arquivo de snippets "c_cpp".`}>
  <Image
...
**Abra** o VSCode no diretório `separate_function` que criamos anteriormente.
...
Dentro desse arquivo, digite `clang-format` e pressione <Kbd linux={"Enter"} />.

<Figure caption={`Utilizando o snippet "clang-format".`}>
  <Image
    src={useSnippet}
...
Vamos criar arquivos dentro dessa pasta, de forma que o projeto ficará como a seguir.
...
```zsh title="Fedora, WSL ou Mint" frame="terminal"
mkdir -p build
clang main.c node.c -o build/linked_nodes -include node.h
```
...
<Figure caption={`Selecionando a tarefa de compilação do projeto.`}>
  <Image
    src={selectTask}
...
<Figure caption={`Saída da compilação do projeto "linked_nodes".`}>
  <Image
    src={buildOutput}
...
<Figure caption={`Abrindo o arquivo de tarefas do usuário.`}>
  <Image
    src={openUserTasks}
...
<Figure caption={`Selecionando a opção "Others".`}>
  <Image
    src={othersTasks}
...
<Figure caption={`Conteúdo do arquivo de tarefas do perfil "C/C++".`}>
  <Image
    src={defaultTask}
