  ...
  "configurations": [
    {
      "args": [],
      "cwd": "${workspaceFolder}",
      "name": "LLDB: build and launch C++ project (all files in root) (Local)",
      "preLaunchTask": "Clang: build C++ project (all files in root) (Local)",
...
Como alteramos o nome da tarefa de compilação no arquivo `tasks.json` para `Clang: build C++ project (Local)`, temos que atualizar o `launch.json` para refletir essa mudança, como mostrado no [repositório do livro](https://github.com/gabdumal/book_andesito/tree/main/public/code/c_cpp_projects/binary_tree/.vscode).
