...
O [**Git**](https://git-scm.com/) é um sistema de controle de versão distribuído.
...
Para instalar o Git no Fedora &mdash; ou no WSL utilizando o Fedora como distro &mdash; basta executar os comandos abaixo.
...
Para instalar o Git no Mint ou outras distros baseadas no Debian, execute os comandos abaixo.
...
A instalação depende do sistema operacional que você está utilizando, como explicado no [repositório oficial](https://github.com/cli/cli/blob/trunk/docs/install_linux.md#installing-gh-on-linux-and-bsd).
...
1. Escolha o protocolo de autenticação. A escolha mais comum é `HTTPS`. Caso você já tenha definido uma chave SSH no GitHub e saiba o que está fazendo, você pode escolher `SSH`.
1. Quando perguntado sobre autenticar com as credenciais do GitHub, digite <Kbd linux={"y"} />.
...
<Figure caption="Código de autenticação do GitHub CLI colado no navegador.">
  <Image
    src={githubCode}
    alt={
...
<Figure caption="Resultado da autenticação pelo navegador.">
  <Image
    src={ghAuthComplete}
    alt={
