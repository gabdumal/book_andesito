...
Execute o seguinte comando no **Linux** ou no **WSL** para criar uma pasta com o nome `dev` dentro do diretório do nosso usuário.
Dentro dela, vamos criar um conjunto de projetos no decorrer deste guia.
...
:::note
O argumento `-p` após o comando `mkdir` serve para garantir que a pasta ou seja criada ou permaneça como está.
Caso a pasta já exista, o comando simplesmente não faz nada.
:::
...
<Figure caption={`Execução do script "hello".`}>
  <Image
    src={hello}
...
<Figure caption={`Selecionando o perfil "Python" no Visual Studio Code.`}>
  <Image
...
<Figure caption={`Visual Studio Code com funcionalidades para o Python.`}>
  <Image
    src={workspace}
...
Para executar ou depurar um arquivo pela interface do Visual Studio Code, basta selecionar o botão `Run and Debug` na barra lateral.
...
<Figure caption={`Selecionando como executar o código no Visual Studio Code.`}>
  <Image
    src={selectLauncher}
...
<Figure caption={`Depurando o programa pela interface do Visual Studio Code.`}>
  <Image
    src={debug}
