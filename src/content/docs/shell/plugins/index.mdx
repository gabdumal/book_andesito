...
import zshSettingsWithTheFuck from "/public/configuration/zsh/plugins/the_fuck/.zshrc?raw";
...
Para instalá-lo, execute o seguinte comando no Fedora ou no WSL usando o Fedora.

```zsh title="Fedora ou WSL" frame="terminal"
sudo dnf install thefuck -y
```

Ou o seguinte comando no Mint ou em outras distribuições baseadas no Debian.

```zsh title="Mint" frame="terminal"
sudo apt install thefuck -y
```

Da mesma forma, vamos editar o arquivo de configuração do Zsh para habilitar o plugin.
Adicione as seguintes linhas ao fim do arquivo.

<Code
  code={zshSettingsWithTheFuck}
  lang="zsh"
  title="~/.zshrc"
  frame="code"
  ins={[30, 31, 32]}
