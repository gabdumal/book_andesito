...
Após instalar o Zsh, **abra** uma nova janela do terminal.
Pode aparecer para você o texto de boas-vindas.

<Figure caption="Texto de boas-vindas do Zsh no Windows Terminal rodando o Fedora.">
  <Image
    src={welcome}
...
Caso o prompt solicite, confirme a configuração com <Kbd linux={"0"} /> e então <Kbd linux={"Enter"} />.
...
<Figure caption="Script de instalação do Oh My Zsh sendo executado no Fedora WSL.">
  <Image
    src={installOmz}
...
<Figure caption="Abrindo o arquivo de configuração do Zsh no Visual Studio Code.">
  <Image
    src={zshCodeInitial}
...
<Figure caption="Permitindo confiar no arquivo de configuração do Zsh no VSCode.">
  <Image
    src={trustCode}
