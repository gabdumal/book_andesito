...
import settings from "/public/settings/vscode/initial/settings.json?raw";
...
```bash title="Fedora: Ptyxis" frame="terminal"
...
```bash title="Mint: Gnome Terminal" frame="terminal"
...
Clique nele, e, em seguida, clice na opção `Backup and Sync Settings...` do menu que se abrir.
...
<Figure
  caption={`Seção de extensões do visual Studio Code, em que se pode selecionar a extensão "Braziian portuguese - Code Spell Checker" para ser instalada no WSL.`}
  images={[
    {
      src: extensions,
      alt: `Seção de extensões do visual Studio Code, em que se vê a extensão "WSL" instalada e sua descrição breve. Também se vê a extensão "Braziian portuguese - Code Spell Checker" desabilitada, na qual aparece um botão escrito "Install in WSL: FedoraLinux-42".`,
    },
  ]}
/>
...
<Code code={settings} lang="json" title="settings.json" showLineNumbers />
